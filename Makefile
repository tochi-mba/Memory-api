...
run: ## Serve the API on :8009 with reload
	$(UV) run uvicorn memory_api.api.app:create_app --factory --reload --port 8009

# Signed-in gh fetches private client packages; with no session git fetches anonymously.
docker: ## Build the container image
	@GITHUB_TOKEN="$$(gh auth token 2>/dev/null)" docker build --secret id=github_token,env=GITHUB_TOKEN -t memory-api:local .
